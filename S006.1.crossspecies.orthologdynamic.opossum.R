...
#for (ref.species in c("Opossum","Chicken","Platypus")) {
    ...
    s1 = c("4wpc","5wpc","7wpc","8wpc","11wpc","12wpc","13wpc","19wpc","19wpc","20wpc","2ypb","4ypb",
           "25ypb","28ypb","29ypb","32ypb","39ypb","46ypb","50ypb",paste0(53:55,"ypb"),"58ypb")
    stage = data.frame(s1 = s1,
                       s2 = c("4wpc","5wpc","7wpc","8wpc","11wpc","12wpc","13wpc","19wpc","19wpc","20wpc","toddler","toddler",
                         "youngadult","youngadult","youngadult","youngadult","youngmiddle",rep("oldmiddle",4),rep("senior",2)))
    ...
    correlate = data.frame(focal = c("4wpc","5wpc","7wpc","8wpc","11wpc","12wpc","13wpc","19wpc","20wpc",
                                     "toddler","youngadult","youngmiddle","oldmiddle","senior"),
                           ref=c("13","14","16","18","18","20","28","28","35","74","104","134","164","194"))
    
    #ref.species
    ref.express = fread(file.path("/home/qians/MamDC/Data/Seqdata/CardosoKaessmann2019NatureRNAseq",ref.species,paste0(ref.species,".RPKM.txt"))) %>% 
      as.data.frame()
    row.names(ref.express) = ref.express$V1
    ref.express = ref.express[,-1]
    colnames(ref.express) %<>% gsub("13.5","13",.)
    ref.coldata = colnames(ref.express) %>% as.data.frame()
    ref.coldata$stage = ref.coldata$. %>% lapply(.,function(x)strsplit(x,split = ".",fixed = TRUE)[[1]][2]) %>% unlist()
    ref.coldata %<>% dplyr::filter(.,stage %in% correlate$ref)
    ref.coldata$tissue = ref.coldata$. %>% lapply(.,function(x)strsplit(x,split = ".",fixed = TRUE)[[1]][1]) %>% unlist()
    ref.express = ref.express[,ref.coldata$.]  
    all(colnames(ref.express) == ref.coldata$.)
    ref.mbg = apply(ref.express, 1, function(x)tapply(x, paste(ref.coldata$tissue,ref.coldata$stage,sep = "_"), mean)) %>%
      t() %>% as.data.frame()
    ...
    for (i in 1:nrow(correlate)) {
      ...
      onestage = one %>% dplyr::select(chrtype,matches(paste0(c(paste0(as.character(correlate[i,])[1],"$"),
                                                        paste0(as.character(correlate[i,])[2],"$")),collapse = "|")))
    ...
    express$stage %<>% factor(.,levels = c("4wpc","5wpc","7wpc","8wpc","11wpc","12wpc","13wpc","19wpc","20wpc",
                                           "toddler","youngadult","youngmiddle","oldmiddle","senior"))
    cor= cor.test(as.numeric(with(express[express$chrtype=="XX",],tapply(ratio,stage,median))),1:14,method = "spearman")
    p1 = ggplot(express,aes(stage,log2(ratio),fill=chrtype))+geom_boxplot(notch = TRUE,outlier.alpha = 0)+
      ...
      annotate(geom="text", x=3, y=-4, size=5,
               label=paste0("rho=",round(cor$estimate,2),", P=",round(cor$p.value,2)))+
      guides(fill=FALSE)
       ...
       #                                       labels = rev(c("n.s.","*","**","***"))),
       #                             chrtype = factor(rep(c("AA","XX"),length(testP))),levels = c("AA","XX")),
       #           vjust = 1, size=4)
    ...
    express$stage %<>% factor(.,levels = c("4wpc","5wpc","7wpc","8wpc","11wpc","12wpc","13wpc","19wpc","20wpc",
                                           "toddler","youngadult","youngmiddle","senior"))
    cor= cor.test(as.numeric(with(express[express$chrtype=="XX",],tapply(ratio,stage,median))),1:13,method = "spearman")
    ...
    express %>% dplyr::filter(!grepl("wpc",stage)) %>% ggplot(.,aes(stage,log2(ratio),fill=chrtype))+geom_violin()+
      theme_classic()+coord_cartesian(ylim = c(-4,4))+
      xlab("Development stage") +  ylab(paste0("Expression ratio in ",tolower(Tissue)))+
      theme(axis.text.x = element_text(size=10,angle = 45, hjust = 1, vjust = 1),
            axis.title.x =  element_text(size=12),
    ...
    for (i in c(3:8,11:14)) {
      ...
      test =wilcox.test(onestage[onestage$chrtype=="AA","ratio"],onestage[onestage$chrtype=="XX","ratio"])
      testP = c(testP,test$p.value)
      onestage$gene = row.names(onestage)
      onestage$stage = correlate[i,1]
      express = rbind(express,onestage)
    }
    express$stage %<>% factor(.,levels = c("7wpc","8wpc","11wpc","12wpc","13wpc","19wpc",
                                           "youngadult","youngmiddle","oldmiddle","senior"))
    cor= cor.test(as.numeric(with(express[express$chrtype=="XX",],tapply(ratio,stage,median))),1:10,method = "spearman")
    p3 = ggplot(express,aes(stage,log2(ratio),fill=chrtype))+geom_boxplot(notch = TRUE,outlier.alpha = 0)+
      ...
      scale_y_continuous(breaks = c(-4,-2,-1,0,1,2,4),labels =  c(-4,-2,-1,0,1,2,4))+
      annotate(geom="text", x=3, y=-4, size=5,
               label=paste0("rho=",round(cor$estimate,2),", P=",format(cor$p.value,2)))+
      guides(fill=FALSE)
     # geom_text(aes(x,y,label =lab),
     #           data = data.frame(x = 1:length(testP),
     #                             y = 4.1,
    ...
    topptx(p3,
           paste0("~/MamDC/Result/WangKaessmann2020NatureRNARiboseq/Human/Picture/S006.1.Orthologratio.dynamics.human.",ref.species,".",Tissue,".pptx"),
           width = 7,height = 6)
    write.table(express,file = "~/test.txt",col.names = FALSE,sep = "\t",quote = FALSE)
