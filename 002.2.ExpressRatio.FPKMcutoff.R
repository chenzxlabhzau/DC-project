    ...
    for( Species in c("Human","Mouse")) {
      ...
      for (T in c("Brain","Cerebellum","Heart","Kidney","Liver","Testis","Ovary")) {
        ...
        if (Species=="Human") {
          ...
          ratio$group = lapply(ratio$tissue,function(x)strsplit(x,split = "_",fixed = TRUE)[[1]][1]) %>% unlist()
          df = tapply(ratio$`1`, ratio$group, mean) %>% as.data.frame()
          df$time = ratio[match(row.names(df),ratio$group),"time"]
          df = df[order(df$time),]
          df$time2 = row.names(df) 
          df$time2 %<>% factor(.,levels = .)
          #df$time2 = paste0("T",df$time) %>% factor(.,levels = .)
          df
          
          ratio = ratio[order(ratio$time),]
          ratio$tissue %<>% factor(.,levels = unique(.))
        }
        if (Species=="Mouse") {
          ratio$tissue %<>% gsub(paste0(T,"_"),"",.) %>% factor(.,levels = coldata[coldata$condition==T,"name"])
          ratio$time = coldata[match(ratio$tissue,coldata$name),6]
        ...
        for (i in 1:11) {
          print(cor.test(ratio[,i],log10(ratio$time),method = "spearman"))
        }
        cor = cor.test(ratio[,12],ratio$time,method = "spearman")
        ratio %<>% pivot_longer(.,cols=1:12)
        #ratio$tissue %<>% as.character(head(ratio$tissue)) %>% gsub(paste0(T,"_"),"",.)
        cor.test(ratio$value,log10(ratio$time))
        ...
        ggsave(filename = paste0("/home/qians/MamDC/Result/CardosoKaessmann2019NatureRNAseq/",Species,"/Picture/002.",Species,
                                 ".ExpressRatio.FPKM.",T,".pdf"),device = "pdf",width = 12, height = 7)
        eoffice::toffice(p,)
      }
    }
  }
